...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(allProductData);
  } catch (err) {
...
router.post("/", async (req, res) => {
  try {
    const createdProductData = await Product.create(req.body);
    if (req.body.tagIds) {
    ...
    res.status(200).json(createdProductData);
  } catch (err) {
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const destroyProductData = await Product.destroy({
      where: {
        id: req.params.id,
      },
    });

    if (!destroyProductData) {
      res.status(404).json({ message: "No Product found with that id!" });
      return;
    }

    res.status(200).json(destroyProductData);
  } catch (err) {
    res.status(500).json(err);
  }
});
